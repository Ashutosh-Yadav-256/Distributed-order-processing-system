...
echo "✓ Order Created: $ORDER_C_ID. Waiting 3s for failure & compensation..."
sleep 3

ORDER_C_STATUS=$(curl -s -H "Authorization: Bearer $TOKEN" "$GATEWAY_URL/api/v1/orders/$ORDER_C_ID" | grep -o '"status":"[^"]*' | head -1 | cut -d'"' -f4)
echo "✓ Final Order Status: $ORDER_C_STATUS (Expected: CANCELLED)"
